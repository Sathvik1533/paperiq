"""
Milestone 3 parser tests.
Uses real MLRIT question paper text (from actual extracted papers).
No mocks — tests the actual parsing logic against real data.
"""
import pytest
...
# ── Real MLRIT DM paper raw text (Feb 2024 Regular — A6CS08) ─────────────────
...
4 | a) | Draw the Hasse diagram for the partial ordering on the power set e(S) where S={a,b,c} and <= is subset relation. | 5M | CO2 | BL2
...
7 | b) | How many 5 letter words are there where first and last letters are vowels and the middle letters are consonants. | 5M | CO3 | BL2
...
class TestQuestionTypeClassifier:
    def test_classifies_definition(self):
        ...
        assert qt == QuestionType.DEFINITION
        assert conf >= 0.90

    def test_classifies_proof(self):
        qt, conf = classify_question_type("Show that ~P follows from the premises")
        assert qt == QuestionType.PROOF
        assert conf >= 0.90

    def test_classifies_numerical(self):
        qt, conf = classify_question_type("Solve the recurrence relation an - 9an-1 + 20an-2 = 0")
    ...
    def test_classifies_theory(self):
        ...
        assert qt == QuestionType.THEORY
        assert conf >= 0.80

    def test_classifies_numerical_howmany(self):
        qt, conf = classify_question_type("In how many ways can letters be arranged")
        assert qt == QuestionType.NUMERICAL

    def test_returns_unknown_for_empty(self):
        qt, conf = classify_question_type("")
        assert qt == QuestionType.UNKNOWN
        assert conf == 0.0
    ...
    def test_comparison(self):
        qt, conf = classify_question_type("Differentiate between Euler graph and Hamiltonian graph")
...
class TestMarksExtractor:
    def test_extracts_5M(self):
        val, text = extract_marks("Prove the theorem | 5M | CO1")
        assert val == 5

    def test_extracts_10M(self):
        val, text = extract_marks("Solve recurrence relation | 10M | CO4")
        assert val == 10

    def test_extracts_1M(self):
        val, text = extract_marks("Define tautology | 1M | CO1 | BL1")
        assert val == 1

    def test_extracts_marks_word(self):
        val, text = extract_marks("Explain with 7 Marks")
        assert val == 7
    ...
    def test_ignores_invalid_marks(self):
        # 99 is not a valid mark value
        val, text = extract_marks("Question (99M)")
        assert val is None


# ── Question Parser — Full Paper ──────────────────────────────────────────────
...
class TestQuestionParser:
    ...
    def test_question_has_marks(self):
        ...
        marked = [q for q in part_b if q.marks is not None]
        assert len(marked) >= 3

    def test_question_has_type(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        typed = [q for q in result.questions if q.question_type != QuestionType.UNKNOWN]
        assert len(typed) > 0

    def test_or_questions_marked(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        or_questions = [q for q in result.questions if q.is_or_question]
        assert len(or_questions) >= 1

    def test_hasse_diagram_detected(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        texts = [q.question_text.lower() for q in result.questions]
        assert any("hasse" in t for t in texts)

    def test_kruskal_detected(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        texts = [q.question_text.lower() for q in result.questions]
        assert any("kruskal" in t for t in texts)

    def test_recurrence_relation_detected(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        texts = [q.question_text.lower() for q in result.questions]
        assert any("recurrence" in t for t in texts)
    ...
    def test_traceability_paper_id(self):
        ...
        for q in result.questions:
            assert q.paper_id == PAPER_ID

    def test_traceability_raw_snippet(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        # At least half the questions should have raw_snippet set
        with_snippet = [q for q in result.questions if q.raw_snippet]
        assert len(with_snippet) >= len(result.questions) // 2

    def test_question_has_normalized_text(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        for q in result.questions:
            assert q.normalized_text  # must never be empty
            assert q.normalized_text == q.normalized_text.lower()

    def test_question_has_hash(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        for q in result.questions:
            assert len(q.question_hash) == 64  # SHA-256 hex

    def test_parse_confidence_in_range(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        for q in result.questions:
            assert 0.0 <= q.parse_confidence <= 1.0
    ...
    def test_definition_classify_end_to_end(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        definitions = [q for q in result.questions
                       if q.question_type == QuestionType.DEFINITION]
        # "Define propositional logic", "Define permutation", etc.
        assert len(definitions) >= 2

    def test_proof_classify_end_to_end(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        proofs = [q for q in result.questions
                  if q.question_type == QuestionType.PROOF]
        assert len(proofs) >= 1

    def test_co_extracted(self):
        result = self.parser.parse(REAL_PAPER_TEXT, PAPER_ID, SUBJECT_ID)
        with_co = [q for q in result.questions if q.co]
        assert len(with_co) >= 3
