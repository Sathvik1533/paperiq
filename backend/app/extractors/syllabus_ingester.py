"""
Syllabus Ingestion — Gap #2 fix.

syllabus_topics now receives regulation, college_id, branch_id, subject_id
on every row so the readiness scorer can query directly without a join.
"""
import os
import re
        ...
        for pattern in _UNIT_PATTERNS:
            ...
            if m:
                if current_unit is not None:
                    current_unit.topics = [SyllabusTopic(topic_name=t) for t in current_topics if t]
...
def ingest_syllabus(
    ...
    """
    Extract text, parse unit/topic structure, store in syllabi + syllabus_topics.
    Gap #2: every syllabus_topic row carries regulation/subject_id/college_id/branch_id.
    Returns syllabus_id.
    """
    ...
    parsed_units_json = [
        {
            ...
            "topics": [{"name": t.topic_name, "subtopics": t.subtopics} for t in u.topics],
        }
        for u in units
    ]
    ...
    # Resolve college_id from subject if not provided
    if not college_id:
        try:
            sub = db.table("subjects").select("college_id").eq("id", subject_id).single().execute()
            if sub.data:
                college_id = sub.data.get("college_id")
        except Exception:
            pass
    ...
    for unit in units:
        for topic in unit.topics:
            topic_rows.append({
                ...
                # Gap #2 fix: denormalized for direct queries without join
                "regulation"  : regulation,
                "college_id"  : college_id,
                "branch_id"   : branch_id,
                "subject_id"  : subject_id,
    ...
    if topic_rows:
        ...
        log.info(f"[SyllabusIngester] Stored {len(topic_rows)} topics with regulation={regulation}")
