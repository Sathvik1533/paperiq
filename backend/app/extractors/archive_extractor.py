...
class ArchiveExtractor(BaseExtractor):
    """
    Extracts RAR/ZIP archives into a directory.
    Returns list of extracted file paths for downstream per-file extraction.
    Strategy: unar → patoolib → zipfile (ZIP only)
    """
    ...
    def extract_to_dir(self, archive_path: str, dest_dir: str) -> List[str]:
        ...
        raise ExtractionError(f"Could not extract: {archive_path}. Install unar: brew install unar")
    ...
    def _try_unar(self, archive_path: str, dest_dir: str) -> bool:
        ...
        if not unar:
            return False
        try:
            result = subprocess.run([unar, "-o", dest_dir, "-f", archive_path], capture_output=True, text=True, timeout=120)
