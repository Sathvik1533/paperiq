...
Fast, deterministic, no LLM needed for common cases.
Returns (QuestionType, confidence_score).
"""
import re
...
_RULES: list[tuple[QuestionType, float, list[str]]] = [
    ...
    (QuestionType.DEFINITION, 0.95, [
        ...
        r"\bwrite the definition\b", r"\blist the properties\b",
        r"\blist.*properties\b",
    ]),
    (QuestionType.CONSTRUCTION, 0.92, [
        r"\bdraw\b", r"\bconstruct\b", r"\bsketch\b",
        r"\bdraw the hasse\b", r"\bdraw.*diagram\b", r"\bapply kruskal\b",
        r"\bkruskal'?s algorithm\b",
        r"\bfind.*minimum spanning\b",
    ...
    (QuestionType.NUMERICAL, 0.90, [
        ...
        r"\brecurrence relation\b.*\bsolve\b",
        r"\bsolve.*recurrence\b",
        r"\bcharacteristic roots?\b",
    ...
    (QuestionType.THEORY, 0.85, [
        ...
        r"\bstate and explain\b", r"\bstate and prove\b",
        r"\billustrate\b", r"\belaborate\b",
        r"\bwrite.*about\b", r"\bwrite.*note\b",
        r"\bbriefly explain\b", r"\bwith.*example\b",
    ]),
...
def classify_question_type(text: str) -> tuple[QuestionType, float]:
    """
    Returns (QuestionType, confidence) for the given question text.
    Checks the first ~100 chars most heavily (action verb usually at start).
    """
    if not text or not text.strip():
        return QuestionType.UNKNOWN, 0.0

    text_lower = text.lower().strip()
    # Check start of question first (action verb), then full text
    ...
    for qt, confidence, patterns in _COMPILED:
        for pattern in patterns:
            ...
            if pattern.search(text_lower):
                return qt, confidence * 0.85  # slight penalty for match not at start
