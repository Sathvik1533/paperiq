...
export function GuidedTour({ steps, onComplete, onSkip }: Props) {
  ...
  useEffect(() => {
    ...
    if (step.route && location.pathname !== step.route) {
      navigate(step.route)
      setTimeout(locateTarget, step.waitMs || 800)
    } else {
      setTimeout(locateTarget, step.waitMs || 200)
    }
  }, [currentStep])
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <button
            onClick={onSkip}
            style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px', borderRadius: 6 }}
          >
            Skip tour
          </button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
          <div style={{
            ...
            <span className="material-symbols-outlined" style={{ fontSize: 20, color: '#f97316', fontVariationSettings: "'FILL' 1" }}>
              explore
            </span>
          </div>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: '#fff', lineHeight: 1.3, margin: 0 }}>
            {step.title}
          </h3>
        </div>
