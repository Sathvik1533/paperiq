...
export function Papers() {
  ...
  useEffect(() => {
    if (!subjectId) return
    setLoading(true)
    getPapers(Number(subjectId), reg)
      .then(setPapers)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {!subjectId && (
          <div className="bg-gray-800 rounded-xl p-6 text-gray-400 text-center">
            Select a subject from the Search page to view its papers.
          </div>
        )}

        {loading && (
          <div className="text-gray-400 text-center py-12">Loading papers...</div>
        )}
          ...
          {papers.map((paper) => (
            <div key={paper.id} className="bg-gray-800 rounded-xl overflow-hidden">
              ...
              >
                ...
                <span className="text-gray-400 text-sm ml-4">{expanded === paper.id ? '▲' : '▼'}</span>
              </div>

              {expanded === paper.id && paper.parsed_questions && paper.parsed_questions.length > 0 && (
              ...
              {expanded === paper.id && (!paper.parsed_questions || paper.parsed_questions.length === 0) && (
                <div className="border-t border-gray-700 p-4 text-gray-500 text-sm">
                  No parsed questions available for this paper.
                </div>
              )}
            </div>
          ))}
        ...
        {!loading && papers.length === 0 && subjectId && (
          <div className="bg-gray-800 rounded-xl p-6 text-gray-400 text-center">
            No papers found. Try running an analysis first.
          </div>
        )}
      </div>
    </div>
